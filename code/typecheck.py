...
def while_stmt(node):
    ...
    ctype = walk(cond)
    if ctype[0] != 'INT_TYPE':
        raise ValueError("while condition has to be of type INT_TYPE not {}"
                    .format(ctype[0]))
    walk(body)
...
def if_stmt(node):
    ...
    ctype = walk(cond)
    if ctype[0] != 'INT_TYPE':
        raise ValueError("if condition has to be of type INT_TYPE not {}"
                    .format(ctype[0]))
    walk(then_stmt)
    walk(else_stmt)
...
def minus_exp(node):
    ...
    t2 = walk(c2)
    tr = promote(t1,t2)
    if tr[0] not in ['INT_TYPE','FLOAT_TYPE']:
...
def array_access_exp(node):
    ...
    if ix_type[0] != 'INT_TYPE':
        raise ValueError("array index has to be of type INT_TYPE")
