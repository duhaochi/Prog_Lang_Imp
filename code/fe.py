...
# stmt : {VOID_TYPE} VOID_TYPE ID LPAREN ({INT_TYPE,FLOAT_TYPE,STRING_TYPE} formal_args)? RPAREN stmt
#      | {INT_TYPE,FLOAT_TYPE,STRING_TYPE} data_type ID decl_suffix
#      | {ID} ID id_suffix
#      | {GET} GET ID ({SEMI} SEMI)?
#      | {OUT} OUT  exp ({SEMI} SEMI)?
...
# data_type : {INT_TYPE,FLOAT_TYPE,STRING_TYPE} primitive_type
...
# primitive_type : {INT_TYPE} INT_TYPE
#           | {FLOAT_TYPE} FLOAT_TYPE
#           | {STRING_TYPE} STRING_TYPE
def primitive_type(stream):
    if stream.pointer().type in ['INT_TYPE']:
...
# decl_suffix : {LPAREN} LPAREN ({INT_TYPE,FLOAT_TYPE,STRING_TYPE} formal_args)? RPAREN stmt
...
def primary(stream):
    if stream.pointer().type in ['INTEGER']:
        ...
        e = primary(stream)
        # (CONST, TYPE, VAL)
        if e[0] == 'CONST' and e[1][0] == 'INT_TYPE':
            return ('CONST', ('INT_TYPE',), 0 if e[2] else 1)
        else:
            return ('NOT', e)
    else:
...
# formal_args : {INT_TYPE,FLOAT_TYPE,STRING_TYPE} data_type ID ({COMMA} COMMA data_type ID)*
